...
#-------------------------------------------------------------------------------
def NASG_train (NameNet, num_epoch, loader, test_loader, scheduleLR, criterion, record_path, record_name):
    ...
    t = 0
    list_x = {k: f.detach().clone() for k, f in net.named_parameters()}
    list_x_new = {k: f.detach().clone() for k, f in net.named_parameters()}
    ...
    for epoch in range (num_epoch[0]):
        ...
        # Update last iterate --------------------------------------------------
        for name, f in net.named_parameters():
            list_x_new[name] = f.detach().clone()
            f.data = list_x_new[name] * (1 + bias) - list_x[name] * bias
            list_x[name] = list_x_new[name].detach().clone()
    #---------------------------------------------------------------------------
